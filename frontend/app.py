...
def loginin():
    ...
    if request.method == 'GET':
        # 处理获取博客和区块链信息的请求
        # 向后端发送获取博客和区块链信息的请求
        blog_response = requests.get(f'{backend_url}/loginin', headers={'Authorization': session['username']})
        if blog_response.status_code == 200:
            # 解析后端返回的 JSON 数据
            blog_result = blog_response.json()
            # 打印响应头信息
            #print("响应头:", blog_response.headers)
            #打印响应信息
            #print(blog_response.text)

            # 检查响应状态
            if blog_result.get('status') == 'success':
                return jsonify(blog_result)  # 返回 JSON 格式的响应
            else:
        ...
        # 向后端发送写博客的请求
        blog_entry_response = requests.post(f'{backend_url}/loginin', json={'content': content},
                                             headers={'Authorization': session['username']})
        # 接收后端更新的数据
        blog_response = requests.get(f'{backend_url}/loginin', headers={'Authorization': session['username']})
        if blog_response.status_code == 200:
            # 解析后端返回的 JSON 数据
            entry_result = blog_response.json()

            # 检查响应状态
            if entry_result.get('status') == 'success':
                return jsonify(entry_result)  # 返回 JSON 格式的响应
            else:
                return jsonify({'status': 'error', 'message': '写博客失败'})
        else:
            return jsonify({'status': 'error', 'message': '写博客失败，无法连接到后端服务'})
